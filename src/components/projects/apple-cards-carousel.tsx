...
export const Card = ({
  ...
  // Check if src is a URL (for iframe) or image path
  const isUrl = card.src.startsWith('http://') || card.src.startsWith('https://');
  ...
  return (
    <>
        ...
        {open && (
          <div className="fixed inset-0 z-52 h-screen overflow-auto">
            ...
            <motion.div
              ...
              className="relative z-[60] mx-auto my-10 h-fit max-w-6xl rounded-3xl bg-white font-sans dark:bg-neutral-900"
            >
              {/* Sticky close button */}
              ...
              {/* Header section */}
              <div className="relative px-8 pt-2 pb-4 md:px-14">
                <div>
                  <motion.p
      ...
      <motion.button
        layoutId={layout ? `card-${card.title}` : undefined}
        onClick={handleOpen}
        className="relative z-10 flex h-80 w-56 flex-col items-start justify-start overflow-hidden rounded-3xl bg-gray-100 dark:bg-neutral-900 hover:shadow-2xl transition-all duration-300"
      >
        {/* Gradient overlay */}
        <div className="absolute inset-x-0 top-0 z-30 h-full cursor-pointer bg-gradient-to-b from-black/60 via-transparent to-transparent pointer-events-none" />
        
        {/* Title overlay */}
        <div className="absolute z-40 p-8 top-0 left-0 right-0">
          <motion.p
          ...
          <motion.p
            layoutId={layout ? `title-${card.title}` : undefined}
            className="max-w-xs text-left font-sans text-xl font-semibold [text-wrap:balance] text-white md:text-2xl mt-2"
          >
            {card.title}
          </motion.p>
        </div>

        {/* Background content - iframe for URLs, image otherwise */}
        {isUrl ? (
